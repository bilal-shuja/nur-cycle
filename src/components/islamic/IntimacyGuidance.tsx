      ...
      <Card className="relative overflow-hidden card-3d">
        ...
        <CardHeader className="relative z-10">
          <CardTitle className={`flex items-center gap-2 ${settings.darkMode ? 'text-white' : 'text-lavender-800'}`}>
            {getLocalizedText('maritalRelations')}
          </CardTitle>
        </CardHeader>

        <CardContent className="relative z-10 space-y-4">
          <div
            className={`p-4 rounded-lg ${settings.darkMode ? 'bg-slate-800 border border-slate-600' : 'bg-white border border-purple-200'
              }`}
          >
            <h4 className={`font-semibold mb-2 ${settings.darkMode ? 'text-white' : 'text-purple-800'}`}>
              {getLocalizedText('duringMenstruation')}
            </h4>
            <p className={`${settings.darkMode ? 'text-gray-300' : 'text-purple-700'} text-sm`}>
              {getLocalizedText('sexualIntercourseProhibited')}
            </p>
          </div>
          ...
          >
            <h4 className={`font-semibold mb-2 ${settings.darkMode ? 'text-white' : 'text-purple-800'}`}>
              {getLocalizedText('afterPurification')}
            </h4>
            <p className={`${settings.darkMode ? 'text-gray-300' : 'text-purple-700'} text-sm`}>
              {getLocalizedText('afterMenstruationEnds')}
            </p>
          </div>
        </CardContent>
      </Card>
